...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		FlxG.save.bind('chillin' #if (flixel < "5.0.0"), 'techniktil' #end);

		ClientPrefs.loadPrefs();

		/*var trackThing:String = '';
		for(i in 1...102)
		{
			trackThing += '\"music/track_$i.mp3\"';
			if(i != 101) trackThing += ',\n                '; else trackThing += '\n                ';
		}
		trace(trackThing);*/
	...
	function startIntro()
	{
		...
		logoSpr = new FlxSprite(0, FlxG.height * 0.5).loadGraphic(Paths.image('titlelogo'));
		add(logoSpr);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 8:
					addMoreText('the hit game FNF', -60);
