...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Camera Follow Pos', "Value 1: X\nValue 2: Y\n\nThe camera won't change the follow point\nafter using this, for getting it back\nto normal, leave both values blank.\n\nOr, you could put the character's name in Value 1, and it will move to them (Dad, BF, GF)"],
		['Alt Idle Animation', "Sets a specified suffix after the idle animation name.\nYou can use this to trigger 'idle-alt' if you set\nValue 2 to -alt\n\nValue 1: Character to set (Dad, BF or GF)\nValue 2: New suffix (Leave it blank to disable)"],
		['Screen Shake', "Value 1: Camera shake\nValue 2: HUD shake\n\nEvery value works as the following example: \"1, 0.05\".\nThe first number (1) is the duration.\nThe second number (0.05) is the intensity."],
		['Change Character', "Value 1: Character to change (Dad, BF, GF)\nValue 2: New character's name"],
		['Change Scroll Speed', "Value 1: Scroll Speed Multiplier (1 is default)\nValue 2: Time it takes to change fully in seconds."],
		['Glowing Edge Shader', "Used in Serenity\n\nMakes all the characters Glowing Edges, based on their color"],
		['Camera Zoom', "Self explanitory too, isn't it?"],
		['Change Pixel', "Changes the UI to the Pixel Ones"],
	...
	function addSongUI():Void
	{
		...
		var reloadSongJson:FlxButton = new FlxButton(reloadSong.x, saveButton.y + 30, "Reload JSON", function()
		{
			openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function(){
				if(recoverCheck(false, true)) {
					openSubState(new Prompt('There is a Recovered Version of this Song.\n\nWant it instead?', 0, function(){loadJson(_song.song.toLowerCase());}, function(){loadJson(_song.song.toLowerCase()); noRecover = true;}));
				} else {
	...
	*/
	...
	function recoverCheck(minusThingy:Bool, ?automatic:Bool = false):Bool {
		if(noRecover) return false;

		if(automatic) {
			var isItChecked = FileSystem.exists("./assets/data/recover/" + Paths.formatToSongPath(_song.song) + "-" + CoolUtil.difficulties[PlayState.storyDifficulty] + ".json"); //makes shit easier and saves a lot of code
				if(!isItChecked) {
					var doubleChecky = FileSystem.exists("./assets/data/recover/" + Paths.formatToSongPath(_song.song) + ".json"); //yeah, i think i need this
				if(!doubleChecky) {
					return false;
				}
			}
			return true; //oh my god did i just make it automatic for no reason
			//i guess it came into a use somehow
		} else {
			if(minusThingy) {
				return FileSystem.exists("./assets/data/recover/" + Paths.formatToSongPath(_song.song) + "-" + CoolUtil.difficulties[PlayState.storyDifficulty] + ".json");
			} else {
				return FileSystem.exists("./assets/data/recover/" + Paths.formatToSongPath(_song.song) + ".json");
			}
		}
	}
	...
	{
		//please credit me when using this, i want my parents to be proud of me
		if (CoolUtil.difficulties[PlayState.storyDifficulty] != CoolUtil.defaultDifficulty) {
			if(CoolUtil.difficulties[PlayState.storyDifficulty] == null){
				PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase(), recoverCheck(false));
			}else{
				PlayState.SONG = Song.loadFromJson(song.toLowerCase() + "-" + CoolUtil.difficulties[PlayState.storyDifficulty], song.toLowerCase(), recoverCheck(true));
			}
		}else{
		PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase(), recoverCheck(false));
		}
	...
	function autosaveSong():Void
	{
		...
		var songName = Paths.formatToSongPath(_song.song) + "-" + CoolUtil.difficulties[PlayState.storyDifficulty];
		var path = "./assets/data/recover/" + songName.toLowerCase() + ".json";
		
		if (!FileSystem.exists("./assets/data/recover/"))
			FileSystem.createDirectory("./assets/data/recover/");
