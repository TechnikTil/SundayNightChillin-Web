...
class AchievementsMenuState extends MusicBeatState
{
	...
	public var grpAchievements:FlxSpriteGroup;
	...
	override function create()
	{
		...
		if (TitleState.titleJSON.checkerData.enabled)
		{
			...
			checkeredBG.scrollFactor.set();
			add(checkeredBG);
		}

		grpAchievements = new FlxSpriteGroup();
		...
		for (option in options)
		{
			...
			var spr:FlxSprite = new FlxSprite(0, Math.floor(grpAchievements.members.length / MAX_PER_ROW) * 180).loadGraphic(graphic);
	...
	function _changeSelection()
	{
		...
		var maxRows = Math.floor(grpAchievements.members.length / MAX_PER_ROW);
		if(maxRows > 0)
		{
		...
		grpAchievements.forEach(function(spr:FlxSprite) {
			spr.alpha = 0.6;
	...
	override function update(elapsed:Float)
	{
		...
		if(controls.ACCEPT)
		{
			if(onYes)
			{
				...
				state.grpAchievements.members[state.curSelected].loadGraphic(Paths.image('achievements/lockedachievement'));
				state.grpAchievements.members[state.curSelected].antialiasing = ClientPrefs.data.antialiasing;
