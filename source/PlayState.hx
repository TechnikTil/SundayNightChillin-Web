...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuffJoke:Array<Dynamic> = [
		['So Screwed', 0.2], //From 0% to 19%
        ['Suck my Dick', 0.4], //From 20% to 39%
        ['🤓', 0.5], //From 40% to 49%
        ['Go Play With Your Toys', 0.6], //From 50% to 59%
        ['Ehh', 0.69], //From 60% to 68%
        ['Alright Rating', 0.7], //69%
        ['Cool Bro', 0.8], //From 70% to 79%
        ['Great!', 0.9], //From 80% to 89%
        ['The better Cool!!', 1], //From 90% to 99%
        ['HOW THE FUCK!!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];

	public static var useJokeRatings = false; //to use ratingStuffJoke or not

	//event variables
	private var isCameraOnForcedPos:Bool = false;
	private var isNeonLightThere:Bool = false;
	...
	public static var isPaintStage:Bool = false;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'jsonCanButtFuckMeAlright': //Week 1
				var bg:BGSprite = new BGSprite('BG', 0, 0, 1, 1);
				add(bg);

				var sun:BGSprite = new BGSprite('Sun', 0, 0, 1, 1);
				add(sun);

				var clouds:BGSprite = new BGSprite('Clouds', 0, 0, 1, 1);
				clouds.velocity.x = 5;
				add(clouds);

				var mountains:BGSprite = new BGSprite('Mountain', 0, 0, 1, 1);
				add(mountains);
			...
			case 'infinite background test': //run
				...
				var ground = new FlxBackdrop(Paths.image('ground'), XY);
				#else
				var ground = new FlxBackdrop(Paths.image('ground'), 1, 1, true);
				#end
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				case 'chillin', 'spitting-facts':
					jsonCutscene(daSong);

				case 'serenity':
					if(FlxG.random.bool(0.1)) startVideo('json lore - Made with Clipchamp 1'); else startCountdown();
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", true);
		#end
	...
	function jsonCutscene(song:String) {
        var cutsceneHandler:CutsceneHandler = new CutsceneHandler();

		cutsceneHandler.music = 'breakfast';

		dadGroup.alpha = 0.00001;
		camHUD.visible = false;

		var json:FlxSprite = new FlxSprite(dad.x, dad.y);
		json.offset.set(370, -200);
		json.antialiasing = ClientPrefs.globalAntialiasing;
		addBehindDad(json);
		cutsceneHandler.push(json);

		switch(song) {
			case 'chillin':
				json.frames = Paths.getSparrowAtlas('cutscene shit/lazy piece of shit');
				json.animation.addByPrefix('anim', 'stop being fucking lazy', 24, false);

				cutsceneHandler.endTime = 4; // how long the cutscebe runs before it calls the finish callback
				...
				{
					json.animation.play('anim', true);
				});
		
				for (i in 0...7) {
					cutsceneHandler.timer(2.4167 + (i / 10), function()
					{
						moveCamera('bf');
						boyfriend.animation.play('hey', true);
						boyfriend.animation.finishCallback = function(name:String)
				...
				case 'spitting-facts':
					json.frames = Paths.getSparrowAtlas('cutscene shit/WHAT WOW');
					json.animation.addByPrefix('anim', 'shocking', 24, false);

					var cutsceneSnd:FlxSound = new FlxSound().loadEmbedded(Paths.sound('OHHHH'));
					FlxG.sound.list.add(cutsceneSnd);
	
					cutsceneHandler.endTime = 6; // how long the cutscebe runs before it calls the finish callback
					...
					cutsceneHandler.timer(3, function()
					{
						boyfriend.animation.play('scared', true);
					});
		}
	...
	function startSong():Void
	{
		...
		var bufalfuck:String = '';
		if(dad.curCharacter == 'sancicexe') {
			bufalfuck = 'sancicex'; //hardcoded as fuck
		} else if(dad.curCharacter == 'gb') {
			bufalfuck = 'gb'; //hardcoded as fuck
		}

		#if desktop
		// Updating Discord Rich Presence (with Time Left)
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", true, true, songLength);
		#end
	...
	function eventPushed(event:EventNote) {
		switch(event.event) {
			...
			case 'Glowing Edge Shader':
				dadbattleBlack = new BGSprite(null, FlxG.width * -0.5, FlxG.height * -0.5);
				dadbattleBlack.makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.BLACK);
				dadbattleBlack.visible = false;
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer != null && startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", true, true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", true);
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", true);
		}
		#end
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				switch(Paths.formatToSongPath(SONG.song)) {
					case 'spitting-facts' | 'seacest-sauce':
						openSubState(new BetterGameOverSubstate(boyfriend.getScreenPosition().x - boyfriend.positionArray[0], boyfriend.getScreenPosition().y - boyfriend.positionArray[1], camFollowPos.x, camFollowPos.y));
				...
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", true);
				#end
				isDead = true;
				return true;
			}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Camera Follow Pos':
				if(camFollow != null)
				{
					...
					if(!Math.isNaN(Std.parseFloat(value1)) || !Math.isNaN(Std.parseFloat(value2))) {
						...
						trace('Forcing camera on to ' + charType);
						moveCameraSection(charType);
						isCameraOnForcedPos = true;
					} else {
						isCameraOnForcedPos = false; //making sure
					}
				}
			...
			case 'Glowing Edge Shader':
				var glowingEdgeShader:GlowingEdgeShader = new GlowingEdgeShader();
				
				if(!isNeonLightThere) {
					FlxTween.tween(camHUD, {angle: 180}, 0.32, {ease: FlxEase.quartIn});
					FlxTween.tween(camGame, {angle: 180}, 0.3, {ease: FlxEase.quartIn, onComplete: function(twn:FlxTween) {
						dadbattleBlack.visible = true;

						/*for (char in [boyfriend, dad, gf]) {
							char.color = FlxColor.fromRGB(char.healthColorArray[0], char.healthColorArray[1], char.healthColorArray[2], 196);
						}*/

						for (cam in [camHUD, camGame]) {
							cam.setFilters([new ShaderFilter(glowingEdgeShader)]);
							trace(cam);
						}

						FlxTween.tween(camHUD, {angle: 360}, 0.32, {ease: FlxEase.quartOut});
						FlxTween.tween(camGame, {angle: 360}, 0.3, {ease: FlxEase.quartOut});
						isNeonLightThere = true;
					}});
				} else {
					FlxTween.tween(camHUD, {angle: -180}, 0.32, {ease: FlxEase.quartIn});
					FlxTween.tween(camGame, {angle: -180}, 0.3, {ease: FlxEase.quartIn, onComplete: function(twn:FlxTween) {
			...
			case 'Camera Zoom':
				var camThing:Float = Std.parseFloat(value1);
				var camTime:Float = Std.parseFloat(value2);

				FlxTween.tween(FlxG.camera, {zoom: camThing}, camTime, {ease: FlxEase.sineInOut, onComplete: function(twn:FlxTween) {
					defaultCamZoom = camThing;
				}});

			case 'Change Pixel':
				isPixelStage = !isPixelStage;

				for (daThing in [playerStrums, opponentStrums]) {
					for (i in daThing.members) {
						i.reloadNote();
					}
				}

				for (daThing in grpNoteSplashes) {
					var skin = 'noteSplashes';

					if(SONG.splashSkin != null && SONG.splashSkin.length > 0) skin = SONG.splashSkin;
					if(isPixelStage) skin = 'pixelUI/' + skin;

					daThing.loadAnims(skin);
					
				}

				for (note in notes) {
					if (isPixelStage) note.offsetX += 30;
					if (!isPixelStage) note.offsetX -= 30;
					note.reloadNote();
					if(isPixelStage && note.isSustainNote) {
						note.scale.y *= daPixelZoom; //unfuck sustain notes
						note.updateHitbox();

						note.prevNote.scale.y = 19.79208;
					}
				} 
	...
	function moveCameraSection(?forceChar:String = ''):Void {
		if(SONG.notes[curSection] == null || isCameraOnForcedPos) return;

		if(forceChar != '') isCameraOnForcedPos = true;

		if (gf != null && SONG.notes[curSection].gfSection || forceChar.contains('gf'))
		{
		...
		if (!SONG.notes[curSection].mustHitSection || forceChar.contains('dad'))
		{
			moveCamera('dad');
	...
	private function cachePopUpScore()
	{
		...
		} else if(isPaintStage) {
			pixelShitPart1 = 'paintUI/';
			pixelShitPart2 = '-paint';
		}
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			else
			{
				...
				var funkinRating = ratingStuff;
				if(useJokeRatings) funkinRating = ratingStuffJoke;
				
				// Rating Name
				if(ratingPercent >= 1)
				{
					ratingName = funkinRating[funkinRating.length-1][0]; //Uses last string
				}
				else
				{
					for (i in 0...funkinRating.length-1)
					{
						if(ratingPercent < funkinRating[i][1])
						{
							ratingName = funkinRating[i][0];
							break;
						}
					}
				}
